...
#
# adapted from https://github.com/hwang595/distributed_tensorflow/blob/7ec9782bda7d0f35835a75acd4c41eaf39178e04/Distributed_test/tools/tf_ec2.py
...
def tf_ec2_run(argv, configuration):
    ...
    def summarize_running_instances(argv):
        ...
        summarize_instances(ec2.instances.filter(Filters=[{'Name': 'instance-state-name', 'Values': [
    ...
    def terminate_all_requests():
        ...
        if len(spot_request_ids) != 0:
            print("Terminating spot requests: %s" %
                  " ".join([str(x) for x in spot_request_ids]))
        ...
        while not done:
            ...
            states = [x["State"] for x in spot_requests["SpotInstanceRequests"]
                      if x["LaunchSpecification"]["KeyName"] == configuration["key_name"]]
            for state in states:
                if state != "cancelled":
                    done = False
            sleep_a_bit()
    ...
    def terminate_all_instances():
        ...
        if len(all_instance_ids) != 0:
            print("Terminating instances: %s" %
                  (" ".join([str(x) for x in all_instance_ids])))
    ...
    def launch_instances():
        ...
        evaluator_instance_type, evaluator_count = configuration[
            "evaluator_type"], configuration["n_evaluators"]
    ...
    def wait_until_running_instances_initialized():
        ...
        while not done:
            ...
            resps_list = [client.describe_instance_status(
                InstanceIds=ids[i:i + 50]) for i in range(0, len(ids), 50)]
            statuses = []
            for resp in resps_list:
                statuses += [x["InstanceStatus"]["Status"]
                             for x in resp["InstanceStatuses"]]
    ...
    def wait_until_instance_request_status_fulfilled():
        ...
        while not requests_fulfilled or n_active_or_open == 0:
            ...
            print(
                "InstanceRequestId, InstanceType, SpotPrice, State - Status : StatusMessage")
            print("-------------------------------------------")
            n_active_or_open = 0
            for instance_request in statuses["SpotInstanceRequests"]:
                ...
                if state == "active" or state == "open":
                    n_active_or_open += 1
                    print("%s, %s, %s, %s - %s : %s" %
                          (sid, machine_type, price, state, status, status_string))
                    if state != "active":
                        requests_fulfilled = False
            print("-------------------------------------------")
            sleep_a_bit()
    ...
    def connect_client(instance):
        ...
        client.connect(
            host, username=configuration["ssh_username"], key_filename=configuration["path_to_keyfile"])
        return client
    ...
    def check_idle_instances_satisfy_configuration():
        ...
        for k, v in instance_type_to_instance_map.items():
            ...
            if len(v) < n_required:
                print(
                    "Error, running instances failed to satisfy configuration requirements")
                sys.exit(0)
    ...
    def run_tf(argv, batch_size=128, port=1234):
        ...
        for role, requirement in sorted(specs.items(), key=lambda x: x[0]):
            ...
            instances_to_assign, rest = instance_type_to_instance_map[instance_type_for_role][
                :n_instances_needed], instance_type_to_instance_map[instance_type_for_role][n_instances_needed:]
        ...
        worker_host_string = ",".join([x.private_ip_address + ":" + str(port)
                                       for x in machine_assignments["master"] + machine_assignments["worker"]])
        ps_host_string = ",".join(
            [x.private_ip_address + ":" + str(port) for x in machine_assignments["ps"]])
        ...
        command_machine_assignments["master"] = {
        ...
        for name, command_and_machine in setup_machine_assignments.items():
            ...
            print("Pre Command: %s\n" % " ".join(commands))
            t = threading.Thread(
                target=run_ssh_commands_parallel, args=(instance, commands, q))
            t.start()
            threads.append(t)
        ...
        for name, command_and_machine in command_machine_assignments.items():
            ...
            ssh_command = "ssh -i %s %s@%s" % (
                configuration["path_to_keyfile"], configuration["ssh_username"], instance.public_ip_address)
    ...
    def kill_python(argv):
        if len(argv) != 3:
            print(
                "Usage: python inception_ec2.py kill_python instance_id1,instance_id2,id3...")
            sys.exit(0)
    ...
    def download_file(argv):
        if len(argv) != 5:
            print(
                "Usage python inception_ec2.py download_file instances_cluster_string path_to_file dir_to_save")
            sys.exit(0)
        ...
        running_instances = [x for x in ec2.instances.filter(
            Filters=[{'Name': 'instance-state-name', 'Values': ['running']}, {'Name': 'key-name', 'Values': [configuration["key_name"]]}])]
        ...
        if selected_instance == None:
            print("Error, no instance in instance cluster: %s" %
                  cluster_instance_string)
            sys.exit(0)
    ...
    def download_outdir(argv):
        if len(argv) != 4:
            print(
                "Usage python inception_ec2.py download_outdir instances_cluster_string path_to_save")
            sys.exit(0)
    ...
    def run_command(argv, quiet=False):
        if len(argv) != 4:
            print(
                "Usage: python inception_ec2.py run_command instance_id1,instance_id2,id3... command")
            sys.exit(0)
        cluster_instance_string = argv[2]
        command = argv[3]
    ...
    def setup_nfs():
        ...
        live_instances = ec2.instances.filter(Filters=[{'Name': 'instance-state-name', 'Values': ['running']}, {
                                              'Name': 'key-name', 'Values': [configuration["key_name"]]}, {'Name': 'key-name', 'Values': [configuration["key_name"]]}])
        live_instances_string = ",".join(
            [x.instance_id for x in live_instances])
        rm_command = "sudo rm -rf %s" % configuration["nfs_mount_point"]
        argv = ["python", "inception_ec2.py",
                live_instances_string, rm_command]
        ...
        setup_nfs_command = "sudo mount -t nfs4 -o nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2 %s:/ %s" % (
            configuration["nfs_ip_address"], configuration["nfs_mount_point"])
        reduce_permissions_command = "sudo chmod 777 %s " % configuration["nfs_mount_point"]
        command = update_command + " && " + install_nfs_command + " && " + \
            create_mount_command + " && " + setup_nfs_command + \
            " && " + reduce_permissions_command
